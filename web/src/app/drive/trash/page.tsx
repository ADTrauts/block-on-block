...
export default function TrashPage() {
  ...
  }, [status, session?.accessToken, refreshTrash]);
  ...
  const driveItems: TrashedItem[] = trashedItems.filter(
    (item) =>
      item.moduleId === 'drive' &&
  ...
  return (
    <div className="flex h-screen bg-gray-50">
      ...
      <main 
        ...
        onDragLeave={(e) => {
          // Only clear if we're leaving the main element, not a child
      ...
      >
        {trashLoading || status === 'loading' ? (
          ...
          <div className="p-6">
            <Alert type="error" title="Error">{error || trashError}</Alert>
          </div>
        ) : (
          <div className="p-6">
