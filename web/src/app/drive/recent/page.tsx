...
const RecentPage = () => {
  ...
  const renderActivityText = (activity: Activity) => {
    ...
    const fileHref = '/drive?file=' + activity.file.id;
    const linkClassName = "font-semibold text-blue-600 hover:underline";

    switch (activity.type) {
      case 'create':
        return <>{user} created <Link href={fileHref} className={linkClassName}>{activity.file.name}</Link> • {timeAgo}</>;
      case 'edit':
        return <>{user} edited <Link href={fileHref} className={linkClassName}>{activity.file.name}</Link> • {timeAgo}</>;
      case 'delete':
        return <>{user} moved <span className="font-semibold">{activity.file.name}</span> to trash • {timeAgo}</>;
      case 'share':
        return <>{user} shared <Link href={fileHref} className={linkClassName}>{activity.file.name}</Link> • {timeAgo}</>;
      case 'download':
        return <>{user} downloaded <Link href={fileHref} className={linkClassName}>{activity.file.name}</Link> • {timeAgo}</>;
      default:
        return <>Action performed on <Link href={fileHref} className={linkClassName}>{activity.file.name}</Link> • {timeAgo}</>;
    }
  };
      ...
      <div className="flex-1 overflow-auto">
        ...
        <div className="space-y-3">
          {activities.map((activity) => {
            ...
            const iconContainerClass = 'flex-shrink-0 w-10 h-10 rounded-full flex items-center justify-center ' + iconColorClass;
            return (
            <div 
              key={activity.id} 
